...
#################
# Measurement auto-archive script by Martin Zoller
...
datadir = 'd:/buchillon'#'C:/inetpub/wwwroot/meteolac/campbell_buchillon'
datafiles = ['Buchillon_KT1585', 'Buchillon_CPIStatus', 'Buchillon_LoggerSensor', 'Buchillon_WeatherSensor']
dataext = '.dat'
    ...
    with open(tempfile, 'rb') as csvhandle:
        ...
        for row in rdr:
            ...
            if not os.path.exists(yeardir):
                os.makedirs(yeardir)
            monfile = yeardir + '/' + myfile + datetime.strftime(rdate, '_%Y-%m.dat')
            if destfile != monfile:
                try:
                    desthandle.close()
                except NameError:
